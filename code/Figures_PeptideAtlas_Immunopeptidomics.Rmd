...
get_pvals_holms <- function(df) {
  # Determine p-values with Bonferroni–Holm correction
  #
  # Args:
...
# Determine Bonferroni–Holm corrected p-values (corrected separately for
#  CDS and ncORFs)
p_vals_violin <-
  bind_rows(
...
# Figure S4 - HLA class and ncORF peptide origin
## Figure S4A - Peptide classification
```{r}
...
figure_s4a <- p_overview_1 + p_overview_2a +
  plot_spacer() + p_overview_2b +
  plot_layout(widths = c(2, 2))

ggsave("Figure_S4A.svg",
  device = "svg", path = plot_dir, figure_s4a,
  width = 2000, height = 1300, units = "px"
)
```

## Figure S4B - Classification on protein/ORF level
```{r}
...
figure_s4b <- figure_s4b1 + figure_s4b2
```

## Figure S4C - Classification by peptide length
```{r}
# Count peptides >14 aa and ≤ 14aa per HLA class for ncORF and CDS
...
## Figure S4D - Graph length distribution per dataset
```{r}
...
## Figure S4E - Compare HLA I vs II
```{r}
...
## Figure S4F - Check HLA-II peptides
```{r}
...
## Figure S4B-F - Combine

```{r}
figure_s4bf <- figure_s4b + figure_s4c + figure_s4d + figure_s4e +
  free(figure_s4f) +
  plot_layout(
    design = "ABFFF\nCCFFF\nDDFFF\nEEFFF",
    guides = "collect"
...
ggsave("figure_S4BF.svg",
  device = "svg", path = plot_dir, figure_s4bf,
  width = 3200, height = 2500, units = "px"
)
```

# Figure S5 - ORF statistics
## Figure S5A-D - ORF overview
```{r}
...
figure_s5ad <- figure_s5a + figure_s5b + figure_s5c + figure_s5d +
  plot_layout(nrow = 1)

ggsave("Figure_S5AD.svg",
  device = "svg", path = plot_dir, figure_s5ad,
  width = 2340, height = 1000, units = "px"
)
```

## Figure S5E - Compare cancer vs non-cancer with cancer genes
```{r}
...
ggsave("Figure_S5E.svg",
  device = "svg", path = plot_dir, figure_s5e,
  width = 1400, height = 1200, units = "px"
)
```

## Figure S5F - Compare on peptide level
```{r}
...
figure_s5f <- wrap_plots(
  list(figure_s5f1, figure_s5f2, figure_s5f3, figure_s5f4),
  ncol = 2, nrow = 2
)

ggsave("Figure_S5F.svg",
  device = "svg", path = plot_dir, figure_s5f,
  width = 1200, height = 1200, units = "px"
)
```

# Figure S6 - Hydrophobicity and expression
## Figure S6C - Hydrophobicity at ncORF ends
```{r}
h_lab <- "Hydrophobicity\n(Kyte-Doolittle)"
...
figure_s6c_plots <- lapply(1:2, function(i) {
  cat <- h_cats[i]
  plot_data <- seq_stats_properties %>%
    filter(pro_cat == cat) %>%
    mutate(y_val = hydro)
...
figure_s6c_plots[[2]] <- figure_s6c_plots[[2]] + remove_y
figure_s6c <- wrap_plots(figure_s6c_plots, ncol = 2)

ggsave("Figure_S6C.svg",
  device = "svg", path = plot_dir, figure_s6c,
  width = 600, height = 300, units = "px"
)
```

## Figure S6D - Hydrophobicity per biotype
```{r}
...
ggsave("figure_S6D.svg",
  device = "svg", path = plot_dir, figure_s6d,
  width = 2516.16, height = 900, units = "px"
)
```

## Figure S6E - Expression per biotype
```{r}
...
figure_s6e <- wrap_plots(gtex_violins_f, nrow = 1)

ggsave("figure_S6E.svg",
  device = "svg", path = plot_dir, figure_s6e,
  width = 2300, height = 600, units = "px"
)
```

## Figure S6F - Expression per tissue
```{r}
...
orf_gtex_tissue_n <- orf_gtex_tissue %>% 
  dplyr::count(tissue, detected) 
...
figure_s6f <- ggplot(orf_gtex_tissue, aes(detected, fpkm, color = detected)) +
...
ggsave("Figure_S6F.svg",
  device = "svg", path = plot_dir, figure_s6f,
  width = 2300, height = 600, units = "px"
)
```
