...
orbl_ncorf_file <- file.path(raw_dir, "VanHeesch_EDtable11.xlsx")
...
```{r}
# ORBL scores
orbl_ncorfs <- read_excel(orbl_ncorf_file, skip = 5, na = "NA") %>%
  mutate(
    BiotypeV42 = str_replace(BiotypeV42, "-", " ") %>%
      factor(orf_order),
    url = str_replace_all(CodAlignView, '=hyperlink\\("', "") %>%
      str_replace('", "CodAlignView"\\)', ""),
    ORBLq = as.numeric(ORBLq),
    ORBLq_primate = as.numeric(ORBLq_primate)
  )
...
orbl_phylocsf <- orbl_ncorfs %>%
  ...
  mutate(
    ...
    is_90 = ORBLq > .9,
    phylocsf_codon = phylocsf / NumCodonsWithStop
  )
```
...
ggsave("figure_4EF.svg", figure_4ef,
  device = "svg", path = plot_dir,
...
figure_s7a <- ggplot(orbl_ref, aes(score, orblq)) +
  geom_line() +
  scale_x_continuous(
    name = "ORBLv of matched ORFs",
    breaks = seq(0, 1, .2),
...
ggsave("Figure_S7A.svg",
  device = "svg", path = plot_dir, figure_s7a,
  width = 60, height = 60, units = "mm"
)
```
...
## Figure S7B-E
```{r}
figure_s7b <- read_orbl_data(orblv_placental_file) %>%
  make_orbl_plot("Placental mammal ORBLv ncORF and MANE Select CDS",
    orbl_col[1:10],
    show_legend = TRUE
  )

figure_s7c <- read_orbl_data(orblv_primate_file) %>%
...
figure_s7be <- figure_s7b + figure_s7c + figure_s7d +
  figure_s7e + plot_layout(nrow = 2, guides = "collect")

ggsave("Figure_S7BE.svg",
  device = "svg", path = plot_dir, figure_s7be,
  width = 250, height = 120, units = "mm"
)
```
...
ggsave("Figure_S7F.svg", figure_s7f,
  device = "svg", path = plot_dir,
