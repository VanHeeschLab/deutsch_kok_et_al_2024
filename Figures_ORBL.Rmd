...
sandmann_data <- read_excel(sandmann_file) %>%
  mutate(
    phylocsf = as.numeric(`PhyloCSF mammals`),
    phylo_cat = case_when(
      is.na(phylocsf) ~ "Undefined",
      phylocsf > 0 ~ "PhyloCSF > 0",
      phylocsf <= 0 ~ "PhyloCSF ≤ 0"
    ),
    phylo_col = case_when(
      is.na(phylocsf) ~ "white",
      phylocsf > 0 ~ green_col[3],
      phylocsf <= 0 ~ grey_col[9]
    )
  )
