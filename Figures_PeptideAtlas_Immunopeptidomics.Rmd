...
# Get Riboseq quality from table S5
...
process_data <- function(data, filter_condition, offset_col, numerator,
  ...
  data %>%
    ...
    pivot_wider(
      names_from = !!sym(offset_col), values_from = n,
      values_fill = 0
    ) %>%
...
# Figure S3 - HLA class and ncORF peptide origin
## Figure S3A - Peptide classification
```{r}
...
figure_s3a <- p_overview_1 + p_overview_2a +
  plot_spacer() + p_overview_2b +
  plot_layout(widths = c(2, 2))

ggsave("Figure_S3A.svg",
  device = "svg", path = plot_dir, figure_s3a,
  width = 2000, height = 1300, units = "px"
)
```

## Figure S3B - Classification on protein/ORF level
```{r}
...
figure_s3b <- figure_s3b1 + figure_s3b2
```

## Figure S3C - Classification by peptide length
```{r}
...
## Figure S3D - Graph length distribution per dataset
```{r}
...
## Figure S3F - Check HLA-II peptides
...
## Figure S3B-F - Combine

```{r}
figure_s3bf <- figure_s3b + figure_s3c + figure_s3d + figure_s3e +
  free(figure_s3f) +
  plot_layout(
    design = "ABFFF\nCCFFF\nDDFFF\nEEFFF",
    guides = "collect"
...
ggsave("figure_S3BF.svg",
  device = "svg", path = plot_dir, figure_s3bf,
  width = 3200, height = 2500, units = "px"
)
```

# Figure S4 - ORF statistics
## Figure S4A-D - ORF overview
```{r}
...
figure_s4ad <- figure_s4a + figure_s4b + figure_s4c + figure_s4d +
  plot_layout(nrow = 1)

ggsave("Figure_S4AD.svg",
  device = "svg", path = plot_dir, figure_s4ad,
  width = 2340, height = 1000, units = "px"
)
```

## Figure S4E - Compare cancer vs non-cancer with cancer genes
```{r}
...
## Figure S4F - Compare on peptide level
```{r}
...
figure_s4f <- wrap_plots(
  list(figure_s4f1, figure_s4f2, figure_s4f3, figure_s4f4),
  ncol = 2, nrow = 2
)
...
# Figure S5 - Hydrophobicity and expression
## Figure S5C - Hydrophobicity at ncORF ends
```{r}
h_lab <- "Hydrophobicity\n(Kyte-Doolittle)"
...
figure_s5c_plots <- lapply(1:2, function(i) {
  cat <- h_cats[i]
  plot_data <- seq_stats_properties %>%
    filter(pro_cat == cat) %>%
    mutate(y_val = hydro)
...
figure_s5c_plots[[2]] <- figure_s5c_plots[[2]] + remove_y
figure_s5c <- wrap_plots(figure_s5c_plots, ncol = 2)

ggsave("Figure_S5C.svg",
  device = "svg", path = plot_dir, figure_s5c,
  width = 600, height = 300, units = "px"
)
```

## Figure S5D - Hydrophobicity per biotype
```{r}
...
ggsave("figure_S5D.svg",
  device = "svg", path = plot_dir, figure_s5d,
  width = 2516.16, height = 900, units = "px"
)
```

## Figure S5E - Expression per biotype
```{r}
...
figure_s5e <- wrap_plots(gtex_violins_f, nrow = 1)
...
## Figure S5F - Expression per tissue
```{r}
...
figure_s5f <- ggplot(orf_gtex_tissue, aes(detected, fpkm, color = detected)) +
